...
Voilà déjà quelques temps que je lorgnait sur Grunt et Gulp pour améliorer mon workflow. Plutôt Gulp d'ailleurs, qui semble avoir pris l'avantage sur Grunt grâce à un système de flux et à une configuration mois verbeuse.  
...
Gulp est un outil basé sur [NodeJS](http://nodejs.org/), il faudra donc avant tout l'installer si ce n'est pas déjà fait. Leur site propose de télécharger des installeurs tout prêt, il n'y a rien de compliqué.
...
{:.center}
![Exemple de tâches Gulp]({{ site.url }}/images/introduction-a-gulp/tâches-gulp.png)

Il y a 5 fonctions de base à connaître avec Gulp, le reste ne sera que plugins :

- `gulp.task()` : défini une tâche exécutable
...
Pas grand chose ne chance, si ce n'est qu'on utilise d'autres plugins, et que cette fois-ci nous avons plusieurs fichiers en entrée. À ce propos d'ailleurs, vous pouvez utiliser quelques éléments de regex dans les paths fournis à `src()` (je n'ai pas encore été voir si ce sont des regex simplifiées, ou carrément des PCRE) :
...
Vos tâches auront besoin de plugins Gulp pour fonctionner. Ces plugins ne sont pas grand chose de plus que des wrappers permettant d'utiliser des outils à travers Gulp.  
...
Notez au passage qu'on a utilisé deux variables dans notre tâche, qui sont simplement des paths qu'on aura défini dans le fichier :
...
La troisième et dernière manière permet d'exécuter les tâches automatiquement, dès qu'un changement est détecté sur un fichier (*losanges rouges sur le schéma*). Ceci sera possible grâce à la fonction `watch()` donc nous avons parlé précédemment, et qui devrait parler aux utilisateur de Sass/LESS entre autres.  
